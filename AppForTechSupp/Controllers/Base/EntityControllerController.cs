...
namespace MvcBaseApp.Controllers
{
    ...
    public abstract class EntityControllerController<T> : Controller where T: class
    {
        ...
        protected abstract void FillModel(IndexGridModel<List<T>> model);

        protected IndexGridModel<List<T>> CreateIndexModel()
        {
            var model = new IndexGridModel<List<T>>() { EntityName = GetControllerName() };
            FillModel(model);
            return model;
        }
