...
namespace MvcBaseApp.Controllers
{
    public class InPageAutorizeController : Controller
    {
        ...
        {
            //return "";
            var appuser =(DataModel.ApplicationUser) new DataModel.ApplicationUser().Parse(formData);
            var login = DataTypeParser.String(formData["UserName"]);
            var password = DataTypeParser.String(formData["Password"]);
            var passwordDuplicate = DataTypeParser.String(formData["PasswordDuplicate"]);
            if (!password.Equals(passwordDuplicate))
                return "Error: " + "Пароли не совпадают";
            //var userId = DataTypeParser.Int(formData["Id_User"]);
            //if (userId == 0)
            //{
            //    userId = DataTypeParser.Int(formData["Id_User_AddAspNetUsers_VI"]);
            //}
            try
            {
                ...
                appuser.Document = document;
                appuser.Id_Document = 0;
                appuser.Document = null;
                if (document.Filename != null)
                {
                    appuser.Document = document;
                    appuser.Id_Document = document.Id;
                    entities.Document.Add(appuser.Document);
                }
                else
                {
                    appuser.Id_Document = null;
                }
                entities.ApplicationUser.Add(appuser);
                entities.SaveChanges();
                ...
                if (result.Succeeded)
                {
                    ...
                    aspnetUser.Id_ApplicationUser = appuser.Id;
                    entities.SaveChanges();
                    return "OK:" + user.Id;
                }
            ...
            catch (Exception e)
            {
                if (e is DbUpdateException)
                    return "Error: " + Labels.ErrorNullFields;
